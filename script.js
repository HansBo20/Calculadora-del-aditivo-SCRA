...
calcularButton.addEventListener('click', () => {
    ...
    // Crea un gráfico con Chart.js (Plano cartesiano)
    const myChart = new Chart(grafico, {
        type: 'line', // Tipo de gráfico: línea
        data: {
            labels: [], // Eje X: Porcentaje de Aditivo (se llenará en el bucle)
            datasets: [{
                label: 'Resistencia a la Compresión (MPa)',
                data: [], // Eje Y: Resistencia (se llenará en el bucle)
        ...
        options: {
            scales: {
                x: {
                    title: {
                        display: true,
                        text: 'Porcentaje de Aditivo (%)'
                    },
                    min: 0, // Rango mínimo del eje X
                    max: 10, // Rango máximo del eje X
                },
                y: {
                    title: {
                        display: true,
                        text: 'Resistencia a la Compresión (MPa)'
                    },
                    min: 0, // Rango mínimo del eje Y
                    max: 40, // Rango máximo del eje Y
                }
            }
        }
    });

    // Genera puntos para la línea de la ecuación
    for (let x = 0; x <= 10; x += 0.5) { // Incrementa en 0.5 para mayor precisión
        const y = 2.14 * x + 24.43;
        myChart.data.labels.push(x.toFixed(2));
        myChart.data.datasets[0].data.push(y.toFixed(2));
    }
